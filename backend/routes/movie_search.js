...
router.post("/", async (req, res) => {
  ...
  const url_for_total_pages = `https://api.themoviedb.org/3/search/multi?query=${query}&api_key=${
    process.env.MOVIEDB_API_KEY
  }&language=fi-FI&page=${1}`;
  ...
  for (let i = 1; i <= totalPages; i++) {
    urls.push(
      `https://api.themoviedb.org/3/search/multi?query=${query}&api_key=${process.env.MOVIEDB_API_KEY}&language=fi-FI&page=${i}`
    );
  }
