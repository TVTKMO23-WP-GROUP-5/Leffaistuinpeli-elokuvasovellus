...
const sql = {
  GET_MEMBERSINGROUP:
    "SELECT a.username FROM account a INNER JOIN groupmembers gm ON a.idaccount = gm.idaccount WHERE gm.idgroup = $1 AND gm.ismember = TRUE",
  GET_APPLICATION:
    "SELECT a.username FROM account a INNER JOIN groupmembers gm ON a.idaccount = gm.idaccount WHERE gm.idgroup = $1 AND gm.ismember = FALSE",
  UPDATE_ISMEMBER:
    "UPDATE groupmembers SET ismember = TRUE WHERE idgroup = $1 AND idaccount = $2",
  GET_GROUPID:
    "SELECT idgroup, groupname, description FROM groups WHERE owner = $1",
  ...
  DELETE_MEMBERFROMGROUP:
    "DELETE FROM groupmembers WHERE idgroup = $1 AND idaccount = $2",
  DELETE_ALLMEMBERS: "DELETE FROM groupmembers WHERE idgroup = $1",
  DELETE_GROUP: "DELETE FROM groups WHERE idgroup = $1 AND owner = $2",
  INSERT_APPLICATION:
    "INSERT INTO groupmembers (idgroup, idaccount) VALUES ($1, $2)",
  CHECK_USER_OWNER: "SELECT EXISTS (SELECT 1 FROM groups WHERE owner = $1)",
  GET_GROUPSTATUS:
    "SELECT gm.*, g.groupname FROM groupmembers gm INNER JOIN groups g ON gm.idgroup = g.idgroup WHERE gm.idaccount = $1",
};
