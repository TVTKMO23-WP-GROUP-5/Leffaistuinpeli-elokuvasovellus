...
const sql = {
  ...
  GET_OWNERID:
    "SELECT idaccount FROM account INNER JOIN groups on account.username=groups.owner WHERE groups.owner=$1",
  GET_OWNERSNAME: "SELECT owner FROM groups WHERE groupname=$1",
  POST_NEWGROUP:
    "INSERT INTO groups (groupname, description, owner) VALUES ($1, $2, $3)",
...
async function createGroup(groupname, description, owner) {
  let result = await pgPool.query(sql.POST_NEWGROUP, [
    groupname,
    description,
    owner,
  ]);
