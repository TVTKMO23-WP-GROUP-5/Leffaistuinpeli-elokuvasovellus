...
export default function AdminPage() {
  ...
  const handleGroupDelete = (admin, id) => {
    ...
    if (confirmed) {
      axios
        .post("/getmembers/deletegroup", { owner: admin, ID: id })
        .then((response) => {
          setGroupMembers(response.data);
  ...
  const handleApplication = (username, group_id, admin) => {
    ...
    if (confirmed) {
      axios
        .post("/getmembers/handleapplication", {
          user: username,
          ID: group_id,
          admin: admin,
        })
  ...
  return (
    <div className="members">
      ...
      <h3
        style={{ color: "var(--dark-background-color)", textAlign: "center" }}
      >
        RYHMÄT
      </h3>
            ...
            <div className="groupmembers-list" key={index}>
              <div className="group-title">
                <div className="gr-title-descripe">
                  ...
                  <h4 className="group-descripe">
                    {groupMembers.kuvaus[index]}
                  </h4>
                </div>
                <button
                  className="deleteButton"
                  name={groupMembers.name[index]}
                  onClick={() =>
                    handleGroupDelete(user, groupMembers.ID[index])
                  }
                >
                  Poista ryhmä
                </button>
